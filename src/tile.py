...
class TileSkeleton:
    ...
    async def parse(cls, bot, possible_variants, string: str, default_prefix: str | None = None,
        ...
        out = cls()
        print(string, default_prefix, prefix)
        ...
        out.raw_string = f"{out.prefix}_{string}" if out.prefix is not None else string
        out.palette = palette
        last_escaped = False
        ...
        out.name = f"{out.prefix}_{out.name}" if out.prefix is not None and out.name else out.name
        if out.name == "2":
        ...
        for i, (raw_var, split) in enumerate(split_vars):
            if raw_var.startswith("m!"):
                ...
                expanded = await self.bot.macro_handler.parse_macros(mac, "T")
...
def is_adjacent(pos, tile, grid, tile_borders=False) -> bool:
    ...
    if x < 0 or y < 0 or \
        ...
        return tile_borders
    return grid[y, x, z, w].name in joining_tiles
